    ...
    if (ImGui::BeginPopup("PrimitiveControl"))
    {
        ...
        for (const auto& primitive : primitives)
        {
            if (ImGui::Selectable(primitive.Label))
            {
                ...
                case OBJ_BULLET:
                {
                    if (World) // World가 유효한지 확인합니다.
                    {
                        // ABullet 클래스에 정의된 스태틱 함수를 호출하여 총알을 스폰합니다.
                        // 이제 카메라 위치/회전 계산 및 초기 설정은 ABullet::SpawnBulletFromCurrentViewpoint 내부에서 처리됩니다.
                        ABullet* BulletActor = ABullet::SpawnBulletFromCurrentViewpoint(World);

                        // 스폰된 액터를 에디터에서 선택할 수 있도록 할당합니다.
